...
type ApiResult<T = unknown> = ActionResult | ({ success: true } & T);
...
function validateSite(data: SiteFormInput): string | null {
...
function saveImageDataUrl(dataUrl: string, maxMb: number, label: string): ApiResult<{ data: string }> {
...
export async function createSiteService(formData: SiteFormInput): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
  const error = validateSite(formData);
  if (error) return { success: false, error };

  try {
    createSite(await autoFillFavicon(formData));
    refreshHome();
    return { success: true };
  } catch (e) {
...
export async function updateSiteService(id: string, formData: SiteFormInput): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function deleteSiteService(id: string): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
  if (!id) return { success: false, error: "站点 ID 不能为空" };
...
export async function reorderSitesService(items: ReorderInput[]): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function importSitesService(jsonText: string, mode: ImportMode): Promise<ApiResult<{ count: number; format: "homedirplus" | "sunpanel" }>> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function exportSitesService(): Promise<ApiResult<{ data: string }>> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function renameCategoryService(oldName: string, newName: string): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function updateCategorySortService(name: string, sortOrder: number): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function deleteCategoryService(name: string): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
  if (!name) return { success: false, error: "分类名称不能为空" };
...
export async function fetchFaviconService(url: string): Promise<ApiResult<{ data: string }>> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function createShortcutService(key: string, siteId: string): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function deleteShortcutService(id: string): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
...
export async function uploadImageService(dataUrl: string, kind: "background" | "site-icon" | "site-logo"): Promise<ApiResult<{ data: string }>> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
  if (kind === "background") return saveImageDataUrl(dataUrl, 8, "背景图片");
  if (kind === "site-logo") return saveImageDataUrl(dataUrl, 4, "主站图标");
  return saveImageDataUrl(dataUrl, 2, "站点图标");
}

export async function updateConfigService(config: Partial<SiteConfig>): Promise<ActionResult> {
  const authErr = await requireAuthResult();
  if (authErr) return authErr;
  ...
  try {
    updateConfig({ ...config, background_blur: clampPercent(config.background_blur, "1") });
    refreshHome();
    return { success: true };
  } catch (e) {
...
export function jsonResult<T extends object>(result: ApiResult<T>) {
  return result.success ? ok(result as T) : fail(result.error, result.error === "未登录" ? 401 : 400);
}
